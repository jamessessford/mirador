(function($) {
  ...
  $.AnnotationTooltip.prototype = {
    ...
    viewerTemplate: $.Handlebars.compile([
      ...
      '<div class="button-container row">',
        '<div class="col-6">',
          '{{#if showUpdate}}<a href="#edit" class="edit btn btn-block primary-button text-white"><i class="fa fa-pencil-square-o fa-fw"></i>{{t "edit"}}</a>{{/if}}',
        '</div>',
        '<div class="col-6">',
          '{{#if showDelete}}<a href="#delete" class="delete btn btn-block secondary-button"><i class="fa fa-trash-o fa-fw"></i>{{t "delete"}}</a>{{/if}}',
        '</div>',
      '</div>',
      '<div class="text-viewer text-center">',
